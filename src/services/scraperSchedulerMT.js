...
class ScraperSchedulerMT {
    constructor() {
        this.isRunning = false;
        this.lastRunDate = null;
        this.schedulerInterval = null;
        this.lockFile = path.join(__dirname, '../../xsmt_scraper.lock');
        this.statusFile = path.join(__dirname, '../../xsmt_scraper.status.json');
    ...
    // Khởi tạo scheduler
    initializeScheduler() {
        console.log('🚀 Khởi tạo Scraper Scheduler cho XSMT...');
    ...
    // Tính toán thời gian chạy tiếp theo (17h14)
    calculateNextRun() {
        const now = new Date();
        const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        const targetTime = new Date(today.getTime() + (17 * 60 + 14) * 60 * 1000); // 17h14

        // Nếu đã qua 17h14 hôm nay, lên lịch cho ngày mai
        if (now >= targetTime) {
    ...
    startScheduler() {
        ...
        console.log(`✅ Scheduler XSMT đã khởi động, kiểm tra thông minh (delay: ${Math.round(initialDelay / 1000)}s)`);
        this.logStatus();
    }
    ...
    async checkAndRun() {
        ...
        try {
            ...
            if (currentHour === 17 && currentMinute === 14) {
                // Kiểm tra xem đã chạy hôm nay chưa
                const today = now.toDateString();
                if (this.lastRunDate === today) {
                    this.logWithDebounce('⏰ Scraper XSMT đã chạy hôm nay, bỏ qua');
                    return;
                }

                // ✅ TỐI ƯU: Kiểm tra lock file với timeout
                if (await this.isLockedWithTimeout()) {
                    this.logWithDebounce('🔒 Scraper XSMT đang chạy, bỏ qua');
                    return;
                }
            ...
            console.error('❌ Lỗi trong scheduler XSMT:', error.message);
    ...
    async isLockedWithTimeout() {
        return new Promise((resolve) => {
            ...
            try {
                ...
                console.error('Lỗi kiểm tra lock file XSMT:', error.message);
                clearTimeout(timeout);
                resolve(false);
            }
        });
    ...
    createLock() {
        try {
            ...
            console.error('Lỗi tạo lock file XSMT:', error.message);
        }
    }
    ...
    removeLock() {
        try {
            ...
            console.error('Lỗi xóa lock file XSMT:', error.message);
        }
    }
    ...
    async runScraper() {
        ...
        try {
            console.log('🎯 Bắt đầu chạy scraper XSMT tự động...');
            ...
            console.log(`📅 Chạy scraper XSMT cho ngày: ${date}, đài: xsmt`);

            // ✅ TỐI ƯU: Chạy scraper với timeout
            const scraperPromise = scrapeXSMT(date, 'xsmt');
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error('Scraper XSMT timeout after 20 minutes')), 20 * 60 * 1000);
            });
            ...
            console.log(`✅ Scraper XSMT hoàn thành thành công trong ${Math.round(duration / 1000)}s`);

        } catch (error) {
            console.error('❌ Lỗi khi chạy scraper XSMT:', error.message);
            this.performanceStats.failedRuns++;
    ...
    getStatus() {
        ...
        try {
            ...
            console.error('Lỗi đọc status file XSMT:', error.message);
            return {
                lastRun: null,
                nextRun: this.calculateNextRun(),
                isRunning: false,
    ...
    updateStatus(updates) {
        try {
            ...
            console.error('Lỗi cập nhật status file XSMT:', error.message);
        }
    }
    ...
    logStatus() {
        ...
        console.log('📊 Trạng thái Scheduler XSMT:');
    ...
    handleGracefulShutdown() {
        const cleanup = () => {
            console.log('🛑 Đang dừng Scheduler XSMT...');
            if (this.schedulerInterval) {
                clearInterval(this.schedulerInterval);
            }
            this.removeLock();
            this.updateStatus({ isRunning: false });
            console.log('✅ Scheduler XSMT đã dừng an toàn');
            process.exit(0);
        };
        ...
        process.on('uncaughtException', (error) => {
            console.error('❌ Uncaught Exception XSMT:', error.message);
            cleanup();
        });
    }
    ...
    stop() {
        ...
        this.removeLock();
        this.updateStatus({ isRunning: false });
        console.log('🛑 Scheduler XSMT đã dừng');
    }

    // Khởi động lại scheduler
    restart() {
        console.log('🔄 Khởi động lại Scheduler XSMT...');
        this.stop();
        this.startScheduler();
    }
}
...
const scraperSchedulerMT = new ScraperSchedulerMT();

module.exports = scraperSchedulerMT; 